...
- `publicPages` (default: `[]`): An array of page routes that can be accessed without passing the `isAccessAllowed`.
...
export default config({
  ui: {
    ...
    redirectMiddleware: [async ({ context, isValidSession} : { context: KeystoneContext, isValidSession: boolean }) = {
      return { to: '...' };
      // return { never: true };
    }],
  },
